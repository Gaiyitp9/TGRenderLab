...
#include "PAL/Windows/WindowHelper.h"
...
namespace TG::PAL
{
    ...
    // 记录不同类型窗口的名称
    static std::unordered_map<WindowCategory, wchar_t const*> gWindowClassName;
    ...
    static int RegisterWindow()
    {
        ...
        gWindowClassName[WindowCategory::Default] = L"Default";

        return 0;
    }

    // 调用RegisterWindow函数
    [[maybe_unused]] static int gRegisterWnd = RegisterWindow();

    wchar_t const* GetWindowClassName(const WindowCategory& type)
    {
        return gWindowClassName.at(type);
    }
    ...
    std::string GetWindowMessageInfo(UINT msg, WPARAM wp, LPARAM lp)
    {
        ...
        const std::string& msgName = it == windowMessage.end() ?
